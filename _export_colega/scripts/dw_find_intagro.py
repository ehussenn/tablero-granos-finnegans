...
OUT = ROOT / "data" / "intagro"; OUT.mkdir(parents=True, exist_ok=True)
...
print("[+] Buscando Intagro en cruce...")
for col in ["destino", "organizacionnombre", "representante", "titular", "corredorprimario", "corredorsecundario"]:
    cur.execute(f"""
        ...
        WHERE UPPER({col}::text) LIKE '%INTAGRO%'
...
cur.execute("""
    ...
    WHERE (UPPER(destino) LIKE '%INTAGRO%' OR UPPER(organizacionnombre) LIKE '%INTAGRO%'
        OR UPPER(corredorprimario) LIKE '%INTAGRO%' OR UPPER(corredorsecundario) LIKE '%INTAGRO%')
...
print(f"\n[+] {len(ctgs)} traslados Intagro desde 01/01/2026")
unicos = list({c.get("numerodocumentoadicional") for c in ctgs if c.get("numerodocumentoadicional")})
print(f"[+] {len(unicos)} CTGs únicos")
(OUT/"intagro_ctgs.json").write_text(json.dumps(ctgs, ensure_ascii=False, indent=2), encoding="utf-8")
print(f"[+] saved -> data/intagro/intagro_ctgs.json")
