"""FYO auto-explorer: login + menúes + sniff."""
...
PROFILE = ROOT / "scripts" / "scraper" / ".fyo_profile"
OUT = ROOT / "scripts" / "scraper" / "out" / "fyo"
...
USER = os.environ.get("FYO_USER", "mloza@agronasaja.com.ar")
PWD = os.environ.get("FYO_PASS", "Trini+22")
...
with sync_playwright() as p:
    ...
    candidates = ["https://fyo.com/", "https://www.fyo.com/", "https://mi.fyo.com/", "https://fyo.com.ar/"]
    landed = None
    for c in candidates:
        try:
            print(f"[+] Probando {c}...")
            ...
            if page.url and "404" not in page.title().lower():
                landed = c; print(f"   OK → {page.url}"); break
        except Exception as e: print(f"   [!] {str(e)[:60]}")

    page.screenshot(path=str(OUT/"01_landing.png"), full_page=True)
    print(f"\n[+] Landed: {page.url}")

    # Buscar link/botón Login
    for sel in ["a:has-text('Iniciar sesión')", "a:has-text('Ingresar')",
                 "button:has-text('Iniciar')", "a:has-text('Login')",
                 "a:has-text('Entrar')", "a[href*='login']", ".btn-login"]:
        try:
            el = page.locator(sel).first
            if el.count() > 0:
                el.click(timeout=4000); print(f"  click {sel}")
    ...
    if page.locator("input[type='password']").count() > 0:
        print("[+] Login...")
        try:
            for u_sel in ["input[type='email']", "input[name='email']", "input[name='username']",
            ...
            print(f"  pass OK")
            for b_sel in ["button:has-text('Iniciar')", "button[type='submit']",
                          "button:has-text('Ingresar')", "input[type='submit']"]:
                el = page.locator(b_sel).first
                if el.count() > 0:
                    el.click(timeout=4000); print(f"  submit"); break
    ...
    print(f"\n[+] {len(unique)} labels únicos en menu:")
    for l in unique[:60]:
    ...
    targets = ["Contratos","Liquidaciones","Fijaciones","Aplicaciones","Movimientos","Descargas",
               ...
               "Calidad","Pagos","Facturas","Resumen","Inicio","Granos","Boletos"]
    print(f"\n[+] Visitando secciones...")
    for w in targets:
        try:
