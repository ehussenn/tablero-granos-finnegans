"""Busca Allaria en DW: como corredor + como destino + extrae CTGs."""
import os, sys, json
...
# Buscar Allaria en cualquier col del cruce
print("[+] Buscando Allaria en cruce...")
for col in ["destino", "organizacionnombre", "representante", "titular", "corredorprimario", "corredorsecundario"]:
    cur.execute(f"""
        ...
        WHERE UPPER({col}::text) LIKE '%ALLARIA%'
...
# Extraer todos los traslados Allaria desde 2026
cur.execute("""
    SELECT * FROM agronasajasrl_traslado_venta_granos_carta_porte_cruce
    WHERE (UPPER(destino) LIKE '%ALLARIA%'
        OR UPPER(organizacionnombre) LIKE '%ALLARIA%'
        OR UPPER(corredorprimario) LIKE '%ALLARIA%'
        OR UPPER(corredorsecundario) LIKE '%ALLARIA%'
        OR UPPER(representante) LIKE '%ALLARIA%')
...
print(f"\n[+] {len(ctgs)} traslados Allaria desde 01/01/2026")
unicos = list({c.get("numerodocumentoadicional") for c in ctgs if c.get("numerodocumentoadicional")})
print(f"[+] {len(unicos)} CTGs únicos")
(OUT/"allaria_ctgs.json").write_text(json.dumps(ctgs, ensure_ascii=False, indent=2), encoding="utf-8")
print(f"[+] saved -> data/allaria/allaria_ctgs.json")
