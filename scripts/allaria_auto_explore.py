"""Allaria Agro auto-explorer: login + menúes + sniff API."""
...
with sync_playwright() as p:
    ...
    print("[+] Goto https://www.allariaagro.com.ar/")
    ...
    # Click ingresar/login si aparece
    for sel in ["a:has-text('Ingresar')", "button:has-text('Ingresar')",
                "a:has-text('Iniciar sesión')", "a:has-text('Login')",
                ".btn-login", "a[href*='login']"]:
        try:
            el = page.locator(sel).first
            if el.count() > 0:
                el.click(timeout=3000); print(f"  click {sel}")
    ...
    if page.locator("input[type='password']").count() > 0:
        ...
        try:
            for u_sel in ["input[name='username']", "input[name='usuario']", "input[type='text']",
                          "input[type='email']", "input[placeholder*='usuario' i]", "input[placeholder*='Usuario' i]"]:
            ...
            print(f"  pass OK")
            for b_sel in ["button:has-text('Ingresar')", "button[type='submit']", "input[type='submit']",
    ...
    print(f"  {len(unique)} labels:")
    for l in unique[:50]:
        print(f"    [{l['text'][:30]:30s}] -> {l['href'][:90]}")

    # Visitar secciones
    targets = ["Contratos","Liquidaciones","Fijaciones","Aplicaciones","Movimientos","Descargas",
               "Mercadería","Operaciones","Saldos","Cuenta corriente","Cartas de porte",
               "Análisis","Calidad","Pagos","Facturas","Granos","Resumen"]
    print(f"\n[+] Visitando secciones...")
    for w in targets:
        try:
