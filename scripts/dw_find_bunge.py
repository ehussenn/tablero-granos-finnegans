...
OUT = ROOT / "data" / "bunge"; OUT.mkdir(parents=True, exist_ok=True)
...
for col in ["destino", "organizacionnombre", "representante", "titular", "corredorprimario", "corredorsecundario"]:
    cur.execute(f"""
        ...
        WHERE UPPER({col}::text) LIKE '%BUNGE%'
...
cur.execute("""
    ...
    WHERE (UPPER(destino) LIKE '%BUNGE%' OR UPPER(organizacionnombre) LIKE '%BUNGE%'
        OR UPPER(corredorprimario) LIKE '%BUNGE%' OR UPPER(corredorsecundario) LIKE '%BUNGE%'
        OR UPPER(representante) LIKE '%BUNGE%')
...
print(f"\n[+] {len(ctgs)} traslados Bunge desde 01/01/2026")
unicos = list({c.get("numerodocumentoadicional") for c in ctgs if c.get("numerodocumentoadicional")})
print(f"[+] {len(unicos)} CTGs únicos")
(OUT/"bunge_ctgs.json").write_text(json.dumps(ctgs, ensure_ascii=False, indent=2), encoding="utf-8")
print(f"[+] saved -> data/bunge/bunge_ctgs.json")
