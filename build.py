...
def fetch_produccion() -> tuple[dict, dict, dict | None]:
    ...
    # ── PRODUCCIÓN AGRONASAJA (según %) — pedido del usuario 02/09 ─────────────────
    # Réplica exacta de la solapa "2 · Producción Agronasaja (según %)" de Información
    # General del Portal de Producción (extranet /vistas/produccion-info):
    #   participación = 1 si el convenio es DIEGO ALVAREZ, sino la participación del lote;
    #   por cultivo: ha cosechadas × p, kg totales × p; avance = ha cosechada×p / ha lote×p.
    # También el panel Avance de Cosecha (general, real sin maíz seg/tardío, y por cultivo).
    prod_agnsj = None
    try:
        if cos:
            def _pctd(a):
                if "DIEGO ALVAREZ" in str(a.get("convenio") or "").upper():
                    return 1.0
                return float(a.get("participacion") or 0)
            _MZP = {"MAIZ SEGUNDA", "MAIZ TARDÍO", "MAIZ TARDIO"}
            pg2, pa2, avc = {}, {}, {}
            haLoteA = haCosA = 0.0
            haT = haC = haTR = haCR = haTP = haCP = 0.0
            for a in cos:
                c = (a.get("cultivo") or "").strip()
                if not c:
                    continue
                hl = float(a.get("haLote") or 0); hc = float(a.get("haCosechada") or 0)
                kg = float(a.get("kgTotales") or 0); p = _pctd(a)
                g = pg2.setdefault(c, {"ha": 0.0, "kg": 0.0})
                g["ha"] += hc; g["kg"] += kg
                q = pa2.setdefault(c, {"ha": 0.0, "kg": 0.0})
                q["ha"] += hc * p; q["kg"] += kg * p
                haLoteA += hl * p; haCosA += hc * p
                haT += hl; haC += hc
                if c.upper().strip() in _MZP:
                    haTP += hl; haCP += hc
                else:
                    haTR += hl; haCR += hc
                v = avc.setdefault(c, {"ha": 0.0, "cos": 0.0, "kg": 0.0})
                v["ha"] += hl; v["cos"] += hc; v["kg"] += kg
            _mz = ("MAIZ TARDÍO", "MAIZ PRIMERA", "MAIZ SEGUNDA")
            _sj = ("SOJA PRIMERA", "SOJA SEGUNDA", "SOJA NO GMO PRIMERA", "SOJA NO GMO SEGUNDA")
            _tot_pa = sum(v["kg"] for v in pa2.values())
            _kg = lambda ks: sum((pa2.get(k) or {}).get("kg", 0) for k in ks)
            prod_agnsj = {
                "kpi": {
                    "ha_agn": haCosA, "ha_lote_a": haLoteA, "ha_pend_a": max(0.0, haLoteA - haCosA),
                    "av_a": (haCosA / haLoteA) if haLoteA > 0 else 0,
                    "mix_maiz": (_kg(_mz) / _tot_pa * 100) if _tot_pa > 0 else 0,
                    "mix_soja": (_kg(_sj) / _tot_pa * 100) if _tot_pa > 0 else 0,
                },
                "gen": {
                    "ha_t": haT, "ha_c": haC, "av": (haC / haT) if haT > 0 else 0,
                    "ha_tr": haTR, "ha_cr": haCR, "av_real": (haCR / haTR) if haTR > 0 else 0,
                    "ha_tp": haTP, "ha_cp": haCP, "av_mzp": (haCP / haTP) if haTP > 0 else 0,
                },
                "cult_pct": pa2,   # {cultivo: {ha, kg}} — parte Agronasaja según %
                "cult_gen": pg2,   # {cultivo: {ha, kg}} — producción general (para % AGNSJ)
                "av_cult": avc,    # {cultivo: {ha (lote), cos (cosechada), kg}}
            }
            print(f"    -> Producción AGNSJ según %: {haCosA:,.0f} ha cosechadas · "
                  f"{_tot_pa/1000:,.0f} tn · avance {prod_agnsj['kpi']['av_a']*100:.0f}% · "
                  f"pend {prod_agnsj['kpi']['ha_pend_a']:,.0f} ha")
    except Exception as e:
        print(f"    [!] producción AGNSJ según %: {e}")
    return out, det, prod_agnsj
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  td.num,.val,.kpi .val,.pos-val{font-variant-numeric:tabular-nums}
  ...
  body.pg-reader #pg-reader-banner{display:flex;align-items:center;gap:12px;padding:10px 14px;background:#fef3c7;border-left:4px solid #D68910;border-radius:8px;color:#854d0e;font-size:13px;margin-bottom:12px}
...
<div class="app-shell">
  ...
  <div class="panel" data-panel="compra">
    ...
    <div class="subpanel active" data-sub-panel="cp-posicion">
      ...
      <div class="filterbar" id="filterbar-cp">
        ...
        <button class="clear" id="cp-pliq" style="background:#B5740E;color:#fff;border-color:#B5740E" title="Ver el pendiente de liquidar por contrato: fijado / sin fijar, cliente, cultivo, campaña">📑 Pend. de liquidar</button>
    ...
    <div class="subpanel" data-sub-panel="cp-cierre-cli">
      <div class="section" style="background:linear-gradient(135deg,#eff6ff,#dbeafe);border:1px solid #93c5fd">
        ...
        <p style="margin:6px 0 0;font-size:12.5px;color:var(--muted)">
          ...
          Clic en una fila azul despliega las entregas · clic en un encabezado ordena la columna (asc/desc).</p>
      </div>
      <div id="tc-cards" style="display:flex;flex-wrap:wrap;gap:10px;margin:10px 0"></div>
      <div style="display:flex;gap:6px;margin:6px 0 0">
        <button class="tc-tab" data-tc="C" style="font-weight:700;padding:7px 14px;border:1px solid var(--line);border-radius:8px;background:var(--bg2);color:var(--ink);cursor:pointer">COMPRA por contrato</button>
        <button class="tc-tab" data-tc="V" style="padding:7px 14px;border:1px solid var(--line);border-radius:8px;background:var(--bg2);color:var(--ink);cursor:pointer">VENTA (solo camiones de tus compras)</button>
      ...
      <div class="filterbar">
        ...
        <div><label>MES FIJACIÓN</label><select id="tc-mes"><option value="">Todos</option></select></div>
        <div><label>FUENTE GASTOS</label><select id="tc-fu"><option value="">Todas</option></select></div>
        <div><label>BUSCAR</label><input id="tc-txt" placeholder="CTG / traslado / texto" style="padding:6px 8px;border:1px solid var(--line);border-radius:8px;background:var(--bg2);color:var(--ink)"></div>
        <div style="display:flex;gap:6px;align-items:flex-end">
          <button id="tc-exp" style="padding:6px 12px;border:1px solid var(--line);border-radius:8px;background:var(--bg2);color:var(--ink);cursor:pointer">Expandir</button>
          <button id="tc-col" style="padding:6px 12px;border:1px solid var(--line);border-radius:8px;background:var(--bg2);color:var(--ink);cursor:pointer">Colapsar</button>
          <button id="tc-clr" style="padding:6px 12px;border:1px solid var(--line);border-radius:8px;background:var(--bg2);color:var(--ink);cursor:pointer">Limpiar</button>
        </div>
      </div>
      <div class="section">
        <div class="tbl-wrap" style="max-height:640px">
          <table class="tbl" id="tc-tbl"><thead id="tc-head"></thead><tbody id="tc-body"></tbody><tfoot id="tc-foot"></tfoot></table>
        </div>
      </div>
      <div class="section" style="font-size:12px;color:var(--muted)">
        <b>Aclaraciones:</b>
        ① Dólar = precio $ ÷ TC BNA divisa vendedor del día de la <b>fijación</b> (si no hay fijación, del día de la liquidación).
        ② En COMPRA, comisión (RECCOM) y sellado/gastos (GTOSCOM 1,25%) son los que <b>cobra Agronasaja</b> en cada liquidación.
        ③ En VENTA, son los que <b>descuenta el entregador/comprador</b>: salen de las líneas de la propia liquidación; si no las trae, del mayor contable, y si no, del tarifario (columna Fuente).
        ④ Importes "sin gastos" = tn × precio exactos de cada liquidación. Tn de compra y de venta pueden diferir por mermas.
    ...
    <div class="subpanel" data-sub-panel="cp-calc-canje">
      <div class="section">
        ...
        <div style="margin-top:18px;padding:12px;background:#fffbeb;border-radius:10px;border-left:4px solid #D68910;font-size:12.5px;color:#92400e;line-height:1.6">
  ...
  <div class="panel" data-panel="venta">
    ...
    <div class="subpanel active" data-sub-panel="posicion">
      ...
      <div class="filterbar" id="filterbar">
        ...
        <button class="clear" id="vt-pliq" style="background:#B5740E;color:#fff;border-color:#B5740E" title="Ver el pendiente de liquidar por contrato: fijado / sin fijar, anticipos financieros, cliente, cultivo, campaña">📑 Pend. de liquidar</button>
  ...
  <div class="panel nav-internal" data-panel="personal" style="display:none">
    ...
    <div id="mb-readonly-banner" style="display:none;padding:12px 14px;border-radius:10px;background:#fef3c7;border-left:4px solid #D68910;color:#92400e;margin-bottom:14px;font-size:13px">
  ...
  const body = sorted.slice(0, 400).map(r => {
    return `<tr>${VP_COLS.map(c => {
      ...
      if(c.k === "moneda"){
        ...
        return `<td title="${mismatch ? 'Original Finnegans: ' + (r.moneda||'') + ' (detectada por magnitud)' : ''}" style="${mismatch?'color:#B5740E;font-weight:600':''}">${vpEscape(detected)}${mismatch?' ⚠':''}</td>`;
      }
      if(c.num){
...
function finRender(){
  ...
  finChartTop = new Chart(document.getElementById('chart-top-fin'), {
    type:'bar',
    data:{labels: top.map(x=>x[0].length>30?x[0].slice(0,30)+'…':x[0]), datasets:[{label:`Imp. Pdte Liq. (${monedaTop?monedaTop[0]:''})`, data: top.map(x=>x[1]), backgroundColor:'#C0392B', borderRadius:4}]},
...
function cpfRender(){
  ...
  cpfChartTop = new Chart(document.getElementById('chart-top-cpfin'), {
    type:'bar',
    data:{labels: top.map(x=>x[0].length>30?x[0].slice(0,30)+'…':x[0]), datasets:[{label:`Imp. Pdte Pagar (${monedaTop?monedaTop[0]:''})`, data: top.map(x=>x[1]), backgroundColor:'#C0392B', borderRadius:4}]},
    options:{indexAxis:'y', responsive:true, maintainAspectRatio:false, plugins:{legend:{display:true,position:'bottom'}, tooltip:{callbacks:{label:c=>fmt.num(c.parsed.x)}}}, scales:{x:{ticks:{callback:v=>v.toLocaleString('es-AR')}}}}
  });
...
function cjCtosDetalleHtml(r){
  ...
  if(!ctos.length) return '<span style="color:#C0392B;font-weight:600">— sin contratos —</span>';
  ...
  return `<span title="${escapeHtml(tooltip)}" style="font-size:10.5px">
    ...
    ${pendEnt>0 ? `<span style="color:#B5740E"> · pend ${fmt.num(pendEnt)}</span>` : ''}
...
function flDifChip(r){
  ...
  return `<span title="${desc?'descuento':'bonificación'}" style="color:${desc?'#B5740E':'var(--green)'};font-weight:600">${fmt.num2(d)}%</span>`;
}
function flTotChip(r){
  ...
  return `<span title="${desc?'a descontar':'a bonificar'}" style="color:${desc?'#B5740E':'var(--green)'};font-weight:700">${fmt.num2(Math.abs(t))} ${desc?'desc.':'bonif.'}</span>`;
}
...
function flRender(){
  ...
  order.forEach(k=>{
    ...
    const gtotChip = (!gany||Math.abs(gtot)<0.005) ? '' : `<span title="${gtot>0?'a descontar':'a bonificar'}" style="color:${gtot>0?'#B5740E':'var(--green)'};font-weight:700">${fmt.num2(Math.abs(gtot))} ${gtot>0?'desc.':'bonif.'}</span>`;
...
/* ============== CIERRE DE CLIENTES: cruce trigo 25/26 camion x camion (compra vs venta) ============== */
const TC = PAYLOAD.trigo_cruce || {compra: [], venta: [], convenio: {}};
const tcVxCtg = {};
TC.venta.forEach(v => { if(v.ctg && !tcVxCtg[v.ctg]) tcVxCtg[v.ctg] = v; });
TC.compra.forEach(c => { c.v = c.ctg ? tcVxCtg[c.ctg] : null; c.mes = (c.ffij || c.fecha || "").slice(0, 7); });
TC.venta.forEach(v => { v.mes = (v.ffij || v.fecha || "").slice(0, 7); });
const tcCtgs = new Set(TC.compra.map(c => c.ctg).filter(Boolean));
...
const TCX = TC.compra.filter(c => c.v && c.tn > 0).map(c => {
  ...
  return {ctg: c.ctg, cto: c.cto, prov: c.prov, cultivo: c.cultivo || v.cultivo || "", ctov: v.cto, org: v.org, dest: v.dest, mes: c.mes,
          ...
          comV: v.com * sh, selV: v.sel * sh, otrV: v.otros * sh, gasV: (v.com + v.sel + v.otros) * sh,
          tcC: c.tc || null, tcV: v.tc || null};
});
// gastos dolarizados al TC del dia de cada liquidacion (compra y venta por separado)
TCX.forEach(r => {
  r.comC_u = r.tcC ? r.comC / r.tcC : null; r.selC_u = r.tcC ? r.selC / r.tcC : null;
  r.gasC_u = r.tcC ? r.gasC / r.tcC : null;
  r.comV_u = r.tcV ? r.comV / r.tcV : null; r.selV_u = r.tcV ? r.selV / r.tcV : null;
  r.gasV_u = r.tcV ? r.gasV / r.tcV : null;
  r.dg_u = (r.gasC_u != null && r.gasV_u != null) ? r.gasC_u - r.gasV_u : null;
});
let tcTab = "C", tcAbiertos = new Set(), tcSort = {C: null, V: null, X: null};
const tcFmt = (n, d = 0) => n == null || isNaN(n) ? "—" : n.toLocaleString("es-AR", {minimumFractionDigits: d, maximumFractionDigits: d});
const tcPct = n => n == null || isNaN(n) ? "—" : (n * 100).toLocaleString("es-AR", {minimumFractionDigits: 2, maximumFractionDigits: 2}) + "%";
function tcCard(t, v, s){ return `<div style="background:var(--bg2);border:1px solid var(--line);border-radius:10px;padding:8px 13px;border-top:3px solid var(--blue,#3b82f6);min-width:125px">
  <div style="font-size:10px;text-transform:uppercase;letter-spacing:.5px;color:var(--muted)">${t}</div>
  <div style="font-size:17px;font-weight:800">${v}</div><div style="font-size:11px;color:var(--muted)">${s}</div></div>`; }
function tcOpts(id, vals, lbl){
  const s = document.getElementById(id); if(!s) return; const cur = s.value;
  s.innerHTML = `<option value="">${lbl}</option>` + [...new Set(vals.filter(Boolean))].sort().map(v => `<option>${escapeHtml(v)}</option>`).join("");
  s.value = cur;
}
function tcPasa(r){
  ...
  if(g("tc-mes") && r.mes !== g("tc-mes")) return false;
  if(g("tc-fu")){ const f = tcTab === "C" ? (r.v ? r.v.fu : "") : r.fu; if(f !== g("tc-fu")) return false; }
  if(txt && !JSON.stringify(r).toLowerCase().includes(txt)) return false;
  return true;
}
function tcTh(cols){
  const s = tcSort[tcTab];
  return "<tr>" + cols.map((c, i) =>
    `<th data-tci="${i}" style="cursor:pointer;white-space:nowrap;${c.izq ? 'text-align:left' : ''}" title="clic para ordenar">${c.t}${s && s.i === i ? (s.d > 0 ? " ▲" : " ▼") : ""}</th>`).join("") + "</tr>";
}
function tcOrd(arr, vals){
  const s = tcSort[tcTab];
  if(!s) return arr;
  return [...arr].sort((a, b) => {
    const va = vals(a)[s.i], vb = vals(b)[s.i];
    if(typeof va === "string" || typeof vb === "string") return String(va || "").localeCompare(String(vb || "")) * s.d;
    return ((va || 0) - (vb || 0)) * s.d;
  });
}
function tcSum(a, f){ return a.reduce((s, x) => s + (f(x) || 0), 0); }
function tcRender(){
  ...
  tcOpts("tc-mes", rowsBase.map(r => r.mes), "Todos");
  tcOpts("tc-fu", TC.venta.map(r => r.fu), "Todas");
  const det = `style="color:var(--muted);font-size:11.5px"`;
  ...
  if(tcTab === "C"){
    ...
    const gAgg = cto => {
      const g = G[cto];
      const tn = tcSum(g, r => r.tn), imp = tcSum(g, r => r.tn * r.pc);
      const impU = tcSum(g, r => r.tn * (r.pc_usd || 0)), tnU = tcSum(g, r => r.pc_usd ? r.tn : 0);
    ...
    tcOrd(Object.keys(G).sort(), gVals).forEach(cto => {
      ...
      const dests = [...new Set(a.g.map(r => r.v ? r.v.dest : r.dest).filter(Boolean))];
      const fl = (TC.canje || {})[(a.g[0].prov || "").toUpperCase()] || "";
      const flChip = fl ? ` <span style="font-size:9.5px;font-weight:700;border-radius:8px;padding:1px 6px;background:${fl === "PAGO" ? "#d1fae5" : (fl === "CONVENIO" ? "#fde68a" : "#bfdbfe")};color:#1e3a5f">${fl}</span>` : "";
    ...
    Object.entries(TC.convenio || {}).forEach(([cto, c]) => {
      const fCto = (document.getElementById("tc-cto") || {value: ""}).value;
    ...
    const impUT = tcSum(rows, r => r.tn * (r.pc_usd || 0)), tnUT = tcSum(rows, r => r.pc_usd ? r.tn : 0);
    ...
    tcOrd(Object.keys(G).sort(), gVals).forEach(cto => {
      ...
      const v0 = gVals(cto);
      const dests = [...new Set(g.map(r => r.dest).filter(Boolean))];
      out += `<tr class="tc-grp" data-cto="${escapeHtml(cto)}" style="cursor:pointer;font-weight:700;background:var(--bg2)">
...
document.addEventListener("click", ev => {
  const tb = ev.target.closest(".tc-tab");
  if(tb){ tcTab = tb.dataset.tc; tcAbiertos = new Set(); tcRender(); return; }
  const th = ev.target.closest("#tc-head th[data-tci]");
  if(th){ const i = +th.dataset.tci; const s = tcSort[tcTab];
    tcSort[tcTab] = s && s.i === i ? (s.d > 0 ? {i, d: -1} : null) : {i, d: 1}; tcRender(); return; }
  const gr = ev.target.closest("tr.tc-grp");
...
function pnCalcRow(producto, opsCompra, opsVenta, incluyePlanta){
  ...
  if(origen && pnEsDemsupProd(producto)){
    silobolsaAuto = PN_DS.campo_tn_prod[producto] || 0;
    siloAuto      = PN_DS.semillero_tn_prod[producto] || 0;
    // clasificado "ajustado" de la vista colgado del producto granel (ej. DM 46I20)
    if(PN_DS.clasif_tn_prod[producto] !== undefined) bolsasAuto = PN_DS.clasif_tn_prod[producto];
  }
  if(origen && pnEsDemsupSem(producto)){
    bolsasAuto    = PN_DS.clasif_tn_prod[producto] || 0;
  }
  ...
  const corteBolsa = origen ? (PN_DS.corte_tn_prod[producto] || 0) : 0;
  ...
  // Pend Vincular (regla usuario 01/09/2026, revierte la del 29/08): va del lado de
  // VENTA (demanda) — mercadería entregada al comprador que falta asignar. NO suma
  // a la oferta ni al Pos Pendiente.
  ...
  const prodPendSem  = PN_DS.prod_pend_tn_prod[producto] || 0;
  const prodDespSem  = PN_DS.prod_desp_tn_prod[producto] || 0;
  const prodTotSem   = prodPendSem + prodDespSem;
  // Venta de semilla alineada al DEM-SUP del extranet: O (pendiente), P (despachada), O+P
  const ventaPendSem = PN_DS.venta_pend_tn_prod[producto] || 0;
  const ventaDespSem = PN_DS.venta_desp_tn_prod[producto] || 0;
  ...
  const ofertaTotFinal = ofertaTot;
...
const PN_DEMSUP = PAYLOAD.demsup_soja || null;          // fuente soja (drill-downs)
...
// Bloque DEM-SUP (soja o trigo) para los drill-downs de Planta: granel en campo (col C),
...
function pnDemsupDrillHTML(src, cat, conMargen){
  if(!src) return '';
  const cult = (src.prod_prefix || '').includes('TRIGO') ? 'trigo' : 'soja';
  const nom = cult === 'trigo' ? 'DEM-SUP Trigo' : 'DEM-SUP Soja';
  const cfg = {
    SILOBOLSA: {det:'detalle_campo',       col:'C', titulo:`Semilla ${cult} a granel en campo — ${nom}`,     unidad:'silobolsa(s)'},
    SILO:      {det:'detalle_semillero',   col:'D', titulo:`Semilla ${cult} a granel en semillero — ${nom}`, unidad:'silo(s) de planta'},
    BOLSAS:    {det:'detalle_clasificado', col:'K', titulo:`Semilla ${cult} clasificada (stock) — ${nom}`,   unidad:'producto(s)'},
  }[cat];
  if(!cfg) return '';
  let rows = [];
  (src.variedades||[]).forEach(v => ((src[cfg.det]||{})[v]||[]).forEach(d => rows.push(d)));
  if(!rows.length) return '';
  rows.sort((a,b)=>b.tn-a.tn);
  const totBls = (src.tot_bls||{})[cfg.col] || 0, totTn = (src.tot_tn||{})[cfg.col] || 0;
  ...
  t += `</tbody></table>`;
  t += `<div style="font-size:10.5px;color:var(--muted);margin-top:4px">Fuente: <strong>DEM-SUP Soja · Extranet Agronasaja</strong> (DW Finnegans en vivo, ${escapeHtml(src.campana||'')}). Misma lógica que la vista <em>/vistas/ops-demsup-${cult}</em> del extranet.</div>`;
  return t;
}
...
function pnDrillHTML(type, prods){
  ...
  if(cfg.kind === 'stock'){
    ...
    let demsupHTML = '';
    if(demsupAca){
      let previo = rows.length > 0;
      PN_DEMSUP_SRCS.forEach(src => {
        const pref = (cfg.cat === 'BOLSAS') ? src.prod_prefix_sem : src.prod_prefix;
        const mios = prods.some(p => (p || '').toUpperCase().startsWith(pref));
        if(!mios) return;
        const b = pnDemsupDrillHTML(src, cfg.cat, previo);
        if(b){ demsupHTML += b; previo = true; }
      });
    }
...
function pnRender(){
  ...
  if(incluyePlanta){
    ...
    // Semilla soja/trigo con datos en los DEM-SUP del extranet: entra aunque no tenga
    // contratos en la campaña (si no, variedades sin contrato quedaban afuera).
    Object.keys(PN_DS).forEach(k => Object.keys(PN_DS[k]).forEach(p => prods.add(p)));
  }
  ...
  familias.forEach(fam => {
    ...
    if(famExpanded){
      ...
      granoSem.forEach(prod => { body += pnRenderProdRow(prod, {indent:true, trStyle:'background:#fffbeb;border-left:3px solid #D68910'}); });
...
function tzDetailPlaceholder(r){
  ...
  return `
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-bottom:12px">
      ...
      <div style="padding:10px;background:#fff;border-radius:8px;border-left:3px solid #D68910">
    ...
    if(cargMovs.length){
      ...
      const movRows = cargMovs.slice(0, 10).map(m => {
        ...
        return `<tr>
          ...
          <td style="padding:4px" class="num" style="color:#B5740E">${fmt.num(m.totalDiscount)}</td>
        </tr>`;
      }).join("");
      ...
      if(movsConDetail.length){
        ...
        const kpisDetail = `
          <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:10px">
            ...
            <div style="background:#fee2e2;padding:8px;border-radius:6px;border-left:3px solid #C0392B">
        ...
        const detailBlocks = movsConDetail.slice(0, 20).map(m => {
          ...
          const qaRows = qaWithValues.map(a => {
            ...
            return `<tr>
              ...
              <td style="padding:3px 5px" class="num" style="color:${factor>0?'#B5740E':'var(--muted)'}">${factor||""}</td>
              <td style="padding:3px 5px" class="num" style="color:${a.discount>0?'#C0392B':'var(--muted)'}"><b>${a.discount||""}</b></td>
            </tr>`;
          }).join("");
      ...
      cargillHtml = `
        <div style="margin-top:14px;border-radius:10px;background:linear-gradient(135deg,#fff7ed,#fef3c7);border-left:4px solid #ea580c;padding:14px">
          ...
          <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:10px">
            ...
            <div style="background:#fff;padding:8px;border-radius:6px;border-left:3px solid #B5740E">
              <div style="font-size:10px;font-weight:700;color:var(--muted);text-transform:uppercase">Descuentos Total</div>
              <div style="font-size:15px;font-weight:700;color:#B5740E">${fmt.num(totDisc)} kg</div>
            </div>
  ...
  tbody.innerHTML = rows.map((r, idx) => `
    <tr data-id="${ctEscape(r.id)}">
      ...
      <td style="text-align:center">
        <button class="row-btn" data-id="${ctEscape(r.id)}" data-act="del" style="border:1px solid #fecaca;background:#fff;color:#C0392B;cursor:pointer;padding:3px 9px;border-radius:5px;font-size:11px">🗑️</button>
      </td>
    </tr>
  `).join("");
...
(function tqInit(){
  ...
  function crossRange(){
    ...
    keys.forEach(cer => {
      ...
      if(dupF.length||dupE.length) s+=`<div style="color:#C0392B;font-size:12px">🔴 duplicados en Finnegans: ${dupF.length}${dupF.length?" ("+listC(dupF)+")":""}${dupE.length?` · en extranet: ${dupE.length}`:""}</div>`;
      al+=s+"</div></details>";
    });
    cont.innerHTML=al;
  ...
  function draw(){
    ...
    function gastoCell(num){
      ...
      if(!fg.por_tipo||!fg.por_tipo.length){
        return `<span style="color:#B5740E" title="entregador: ${esc((fg.cerealeras||[]).join(', '))} · falta scrapear sus gastos">— falta extranet</span>`;
      }
    ...
    t.querySelectorAll('.fp-gasto-lnk').forEach(el=>el.addEventListener('click',()=>{
      ...
      const nota=fg.ctgs_sin_datos?`<div style="font-size:11px;color:#B5740E;margin-top:6px">⚠ ${fg.ctgs_sin_datos} CTG de otros entregadores (${esc((fg.cerealeras||[]).join(', '))}) sin gastos scrapeados aún.</div>`:'';
...
def main() -> int:
    ...
    # Cruce trigo 25/26 camion por camion (Cierre de Clientes): generado offline desde las
    # liquidaciones cosechadas de la API + TC BNA divisa vendedor por fecha de fijacion.
    # Se regenera con scratchpad/completar_v4.py + copia a data/tablero_trigo_cruce.json
    tc_path = Path(__file__).resolve().parent / "data" / "tablero_trigo_cruce.json"
    trigo_cruce_data = {"compra": [], "venta": [], "convenio": {}}
    if tc_path.exists():
        try:
            trigo_cruce_data = json.loads(tc_path.read_text(encoding="utf-8"))
            print(f"[+] Cruce trigo 25/26: {len(trigo_cruce_data.get('compra', []))} camiones compra · "
                  f"{len(trigo_cruce_data.get('venta', []))} venta")
        except Exception as e:
            print(f"[!] Error leyendo {tc_path}: {e}")
    ...
    # DEM-SUP Trigo — misma arquitectura que soja (API en vivo de /vistas/ops-demsup-trigo
    # + queries propias al DW para el desglose por producto)
    print(f"\n[+] Bajando DEM-SUP Trigo (fuente Extranet Agronasaja)...", flush=True)
    demsup_trigo_data = None
    try:
        import demsup_trigo
        demsup_trigo_data = demsup_trigo.fetch()
        if demsup_trigo_data:
            _t2 = demsup_trigo_data["tot_tn"]
            print(f"    -> campo {_t2['C']:,.1f} tn · semillero {_t2['D']:,.1f} · clasificado {_t2['K']:,.1f}"
                  f" · corte {_t2['L']:,.1f} · venta desp {_t2['P']:,.1f}")
    except Exception as e:
        print(f"    [!] Error DEM-SUP Trigo: {e}")
